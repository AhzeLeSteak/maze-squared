...
export class CanvasRaycast extends CanvasWebGLTest {
    ...
    drawWalls(lineHeight: number, next_wall: Wall, t: Texture) {
        ...
        const line_start = Math.abs(Math.floor((this.size.y - lineHeight) / 2));
        let last_draw_y = line_start;
        let color = col[0];

        const offset = lineHeight > this.size.y ? .5*(lineHeight - this.size.y)/lineHeight : 0
        const max = lineHeight > this.size.y ? 1 - offset : 1;

        lineHeight = Math.min(lineHeight, this.size.y);

        for (let dy = 0; dy <= lineHeight; dy++) {
            const percent = offset + (max - offset) * dy / lineHeight;
            const color_index = Math.min(col.length - 1, Math.floor(col.length * percent));
