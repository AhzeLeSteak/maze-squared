...
export class CanvasRaycast extends CanvasWebGLTest {
  ...
  drawContext(game: Game, dt: number): void {
    ...
    for (let base_x = 0; base_x < this.size.x; base_x += 1) {
      ...
      const lineHeight = next_wall.distance === Infinity
        ? 0
  ...
  drawWalls(lineHeight: number, next_wall: Wall, t: Texture) {
    ...
    const line_start = Math.max(0, Math.floor((this.size.y - lineHeight) / 2));
    let last_draw_y = line_start;
    let color = col[0];

    const additive = line_start === 0
      ? Math.max(0, Math.floor((lineHeight - this.size.y) / 2))
      : 0;

    const m = Math.min(lineHeight, this.size.y);
    for (let dy = 0; dy <= m; dy++) {
      const y = line_start + dy;
      const col_ratio = (dy + additive) / lineHeight;
      const color_index = Math.min(Math.floor(col.length * col_ratio), col.length-1);
