	...
	drawContext(game: Game): void {
		...
		for (let y = 0; y < map.size.y; y++) {
			for (let x = 0; x < map.size.x; x++) {
				...
				this.drawSquare(x * this.tile_size + 1, y * this.tile_size + 1, this.tile_size - 2, this.tile_size - 2);
				if (box_type > 1) {
		...
		const trunced_pos = {x: Math.floor(player_pos.x), y: Math.floor(player_pos.y)};

		if (trunced_pos.x !== this.last_player_pos.x && trunced_pos.y !== this.last_player_pos.y
		...
		if (environment.highlightPlayerTile) {
		...
		if (environment.drawRays) {
			for (let i = -game.view_angle / 2; i < game.view_angle / 2; i++) {
				for (let j = 0; j < 1; j++) {
					...
					nextWall.from === 'HORIZONTAL' ? this.setColor(55, 200, 40) : this.setColor(50, 85, 255);
					...
					for (const v of nextWall.pointsToDraw) {
						this.drawSquare(v.x * this.tile_size - 2, v.y * this.tile_size - 2, 4, 4);
					}
					for (let s_i = 1; s_i < nextWall.pointsToDraw.length; s_i++) {
						...
						for (let a = 0; a < 4; a++) {
							this.drawLine(p0.x * this.tile_size, p0.y * this.tile_size, p1.x * this.tile_size, p1.y * this.tile_size);
							this.drawLine(p0.x * this.tile_size + a, p0.y * this.tile_size + a,
								p1.x * this.tile_size + a, p1.y * this.tile_size + a);
						}
					}
				}
			}
		...
		const player_size = 8;
		this.drawSquare(player_pos.x - player_size / 2, player_pos.y - player_size / 2, player_size, player_size);


		// affichage de la direction du joueur
		const x_dir = player_pos.x + Math.cos(game.player.angle) * this.tile_size / 2;
		const y_dir = player_pos.y + Math.sin(game.player.angle) * this.tile_size / 2;
		this.drawLine(player_pos.x, player_pos.y, x_dir, y_dir);

		// affichage de l'angle
		this.contextd2D.fillStyle = 'black';
		this.contextd2D.font = '16px Comic sans';
		this.contextd2D.fillText((game.player.angle / degreToRadian + '').substr(0, 5), player_pos.x + 10, player_pos.y + 10);
