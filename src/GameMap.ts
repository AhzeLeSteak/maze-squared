...
export class GameMap {
	...
	getNextPoint({x, y}: Vector2, angle: number): {
		point: Vector2,
		distance: number,
		wallCol: number,
		...
		const directionH = () => angle > pi_over_2 && angle < 3 * pi_over_2 ? 'LEFT' : 'RIGHT';
		...
		try {
			do {
				...
				const diff_x_left_side = nx % 1 + (nx % 1 === 0 && directionH() === 'LEFT' ? 1 : 0);
				const diff_x_right_side = 1 - diff_x_left_side;
				const diff_y_up_side = ny % 1 + (ny % 1 === 0 && directionV() === 'UP' ? 1 : 0);
				const diff_y_down_side = 1 - diff_y_up_side;

				const angle1 = in_range_angle(Math.atan2(diff_y_down_side, diff_x_right_side)); // DOWN RIGHT CORNER
				const angle2 = in_range_angle(Math.atan2(diff_y_down_side, -diff_x_left_side)); // DOWN LEFT CORNER
				const angle3 = in_range_angle(Math.atan2(-diff_y_up_side, -diff_x_left_side)); // UP LEFT CORNER
				const angle4 = in_range_angle(Math.atan2(-diff_y_up_side, diff_x_right_side)); // UP RIGHT CORNER
				...
				if (box_type > 1) {
					const {v, angle: n_angle} = this.getThrough({x: nx, y: ny}, angle, direction);
					nx = v.x;
					ny = v.y;
					old_x = nx;
					old_y = ny;
	...
	public getThrough(v: Vector2, angle: number, direction: 'RIGHT' | 'DOWN' | 'LEFT' | 'UP', isPlayer: boolean = false):
		{
			v: Vector2,
			angle: number
		} {

		const nv = this.getOtherBox(v.x - (v.x % 1 === 0 && direction === 'LEFT' ? 1 : 0),
			v.y - (v.y % 1 === 0 && direction === 'UP' ? 1 : 0));

		const rotation = (this.box(v.x - (v.x % 1 === 0 && direction === 'LEFT' ? 1 : 0),
			v.y - (v.y % 1 === 0 && direction === 'UP' ? 1 : 0)) % 1) / 0.25;
		...
		let logs: any[] = [];
		if (isPlayer && environment.debug) {
			logs = [direction, 'rotation', rotation, diff];
		}
		...
		const dirs: Array<'UP' | 'RIGHT' | 'LEFT' | 'DOWN'> = ['UP', 'RIGHT', 'DOWN', 'LEFT'];
		...
		if (isPlayer && environment.debug) {
			console.log(...logs, 'THEN', direction);
		}
